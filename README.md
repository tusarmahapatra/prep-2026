<h1 align="center">🚀 SDE Preparation 2026</h1>

<p align="center">
  <b>Structured • Measurable • Performance-driven DSA Preparation</b><br/>
  
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Last%20Commit-Automated-success" />
  <img src="https://img.shields.io/badge/Commit%20Activity-Active-blue" />
  <!-- REPO_SIZE_BADGE_START -->
<img src="https://img.shields.io/badge/Repo%20Size-0.044%20MB-blue" />
<!-- REPO_SIZE_BADGE_END -->
  <img src="https://img.shields.io/badge/Preparation-Active-orange" />
</p>

---

## 🎯 Objective

This repository is a **living performance dashboard** for my SDE preparation.

Focus:
- Pattern-based DSA mastery
- Interview-grade reasoning & clean code
- Consistent, trackable progress
- Long-term preparation over shortcuts

---

## 📊 DSA Performance Dashboard
<!-- DASHBOARD_START -->
| Topic | Solved | Target | Progress |
|------|-------|--------|----------|
| Arrays & Strings | 6 | 50 | 🟥 12% |
| Hashmaps | 2 | 40 | 🟥 5% |
| Two Pointers | 2 | 30 | 🟥 6% |
| Stacks & Queues | 2 | 30 | 🟥 6% |
| Linked Lists | 2 | 25 | 🟥 8% |
| Trees | 2 | 50 | 🟥 4% |
<!-- DASHBOARD_END -->

🟥 0–30% 🟨 30–70% 🟩 70–100%

---

## ⚡ Preparation Velocity
<!-- VELOCITY_START -->
| Metric | Value |
|------|------|
| Problems (7 days) | 4 |
| Problems (30 days) | 16 |
| Avg / Day | 0.53 |
| Active Days | 8 |
<!-- VELOCITY_END -->

---

## 📘 Solved Problems Log
<!-- SOLVED_LOG_START -->
| # | Problem | Topic | Pattern | Time | Space |
|--|--------|------|--------|------|-------|
| 1 | Best time to Buy or Sell stock | Arrays & Strings | Prefix Minimum / Running Minimum /State Tracking ?One pass optimisation | O(n) | O(1) |
| 2 | Two Sum | Arrays & Strings | Hashmap | O(n) | O(n) |
| 3 | Contains duplicates | Hashmaps | Hashset | O(n) | O(n) |
| 4 | Valid anagram | Hashmaps | Hashmap | O(n) | O(n) |
| 5 | Valid Palindrome | Two Pointers | Two Pointers | O(n) | O(1) |
| 6 | Two Sum II | Two Pointers | Two Pointers | O(n) | O(1) |
| 7 | Longest substring without repeating characters | Arrays & Strings | Sliding window | O(n) | O(n) |
| 8 | Maximum average subarray | Arrays & Strings | Sliding window | O(n) | O(1) |
| 9 | Range sum query | Arrays & Strings | Prefix Sum | O(n+q) | O(n) |
| 10 | Subarray sum equals k | Arrays & Strings | Hashmap | O(n) | O(n) |
| 11 | Daily Temperatures | Stacks & Queues | Monotonic Stack (Decreasing) | O(n) | O(n) |
| 12 | Valid parentheses | Stacks & Queues | Stack | O(n) | O(n) |
| 13 | Detect Cycle | Linked Lists | Fast and Slow pointers | O(n) | O(1) |
| 14 | Linked List Reversal | Linked Lists | In-place Pointer Reversal | O(n) | O(1) |
| 15 | Inorder Traversal | Trees | DFS | O(n) | O(h) [h: height of tree (balanced tree - O(log n), scewed tree - O(n))] |
| 16 | Level Order Traversal | Trees | BFS | O(n) | O(n) |
<!-- SOLVED_LOG_END -->

---

## 🧠 Pattern Mastery Tracker
<!-- PATTERN_TRACKER_START -->
| Pattern | Count |
|--------|-------|
| Hashmap | 3 |
| Sliding window | 2 |
| Two Pointers | 2 |
| Prefix Minimum / Running Minimum /State Tracking ?One pass optimisation | 1 |
| Prefix Sum | 1 |
| Hashset | 1 |
| Monotonic Stack (Decreasing) | 1 |
| Stack | 1 |
| Fast and Slow pointers | 1 |
| In-place Pointer Reversal | 1 |
| DFS | 1 |
| BFS | 1 |
<!-- PATTERN_TRACKER_END -->

---

## 🧪 Difficulty Breakdown
<!-- DIFFICULTY_START -->
| Difficulty | Count | % |
|-----------|-------|---|
| Easy | 14 | 87.5% |
| Medium | 2 | 12.5% |
<!-- DIFFICULTY_END -->

---

## 📈 Readiness Score
<!-- SCORE_START -->
📈 **Google SDE Readiness Score:** `23 / 100`
<!-- SCORE_END -->

## 🧠 Notes
- Focus on **pattern recognition**
- Avoid brute-force unless needed
- Optimize for **clarity first, micro-optimizations later**
